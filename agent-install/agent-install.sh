...
SUPPORTED_REDHAT_VERSION=(7.6 7.9 8.1 8.2 8.3 8.4 8.5 8 32 $SUPPORTED_REDHAT_VERSION_APPEND)   # compared to what our detect_distro() sets DISTRO_VERSION_NUM to. For fedora versions see https://fedoraproject.org/wiki/Releases,
...
Options/Flags:
    -c, --cert          Path to a certificate file. Default: ./$AGENT_CERT_FILE_DEFAULT, if present. If the 
                        argument begins with 'css:' (e.g. css:$CSS_OBJ_PATH_DEFAULT), it will download the config 
                        file from the MMS. If only 'css:' is specified, the default path $CSS_OBJ_PATH_DEFAULT 
                        will be added. (This flag is equivalent to AGENT_CERT_FILE or HZN_MGMT_HUB_CERT_PATH)
    -k, --config        Path to a configuration file. Default: ./$AGENT_CFG_FILE_DEFAULT, if present. If the 
                        argument begins with 'css:' (e.g. css:$CSS_OBJ_PATH_DEFAULT), it will download the config
                        file from the MMS. If only 'css:' is specified, the default path $CSS_OBJ_PATH_DEFAULT 
                        will be added. All other variables for this script can be specified in the config file, 
                        except for INPUT_FILE_PATH (and HZN_ORG_ID if -i css: is specified). (This flag is 
                        equivalent to AGENT_CFG_FILE)
    -i, --input         Installation packages/files location (default: current directory). If the argument is the URL 
                        of an anax git repo release (e.g. https://github.com/open-horizon/anax/releases/download/v1.2.3) 
                        it will download the appropriate packages/files from there. If it is anax: or 
                        https://github.com/open-horizon/anax/releases , it will default to the latest release. 
                        Otherwise, if the argument begins with 'http' or 'https', it will be used as an APT 
                        repository (for debian hosts). If the argument begins with 'css:' (e.g. 
                        css:$CSS_OBJ_PATH_DEFAULT), it will download the appropriate files/packages from the MMS. 
                        If only 'css:' is specified, the default path $CSS_OBJ_PATH_DEFAULT will be added. (This 
                        flag is equivalent to INPUT_FILE_PATH)
    -z, --tarfile       The name of your agent installation tar file. Default: ./agent-install-files.tar.gz (This
                        flag is equivalent to AGENT_INSTALL_ZIP)
    -j, --pubkey        File location for the public key for an APT repository specified with '-i' (This flag is 
                        equivalent to PKG_APT_KEY)
    -t, --branch        Branch to use in the APT repo specified with -i. Default is 'updates' (This flag is 
                        equivalent to APT_REPO_BRANCH)
    -O, --org           The exchange organization id (This flag is equivalent to HZN_ORG_ID)
    -u, --user          Exchange user authorization credentials (This flag is equivalent to HZN_EXCHANGE_USER_AUTH)
    -a, --auth          Exchange node authorization credentials (This flag is equivalent to HZN_EXCHANGE_NODE_AUTH)
    -d, --node-id       The id to register this node with (This flag is equivalent to HZN_NODE_ID, NODE_ID or 
                        HZN_DEVICE_ID. NODE_ID is deprecated)
    -p, --pattern       Pattern name to register this edge node with. Default: registers node with policy. (This 
                        flag is equivalent to HZN_EXCHANGE_PATTERN)
    -n, --policy        Path to a node policy file (This flag is equivalent to HZN_NODE_POLICY)
    -w, --wait          Wait for this edge service to start executing on this node before this script exits. If 
                        using a pattern, this value can be '*'. (This flag is equivalent to AGENT_WAIT_FOR_SERVICE)
    -T, --timeout       Timeout value (in seconds) for how long to wait for the service to start (This flag is 
                        equivalent to AGENT_REGISTRATION_TIMEOUT)
    -o, --service-org   Specify an org id for the service specified with '-w'. Defaults to the value of HZN_ORG_ID. 
                        (This flag is equivalent to AGENT_WAIT_FOR_SERVICE_ORG)
    -s, --skip          Skip registration, only install the agent (This flag is equivalent to 
                        AGENT_SKIP_REGISTRATION)
    -D, --node-type     Node type of agent being installed: device, cluster. Default: device. (This flag is 
                        equivalent to AGENT_DEPLOY_TYPE)
    -U, --internal-url  Internal url for edge cluster registry. If not specified, this script will auto-detect 
                        the value if it is a small, single-node cluster (e.g. k3s or microk8s). For OCP use: 
                        image-registry.openshift-image-registry.svc:5000. (This flag is equivalent to 
                        INTERNAL_URL_FOR_EDGE_CLUSTER_REGISTRY)
    -l, --log           Logging verbosity level. Display messages at this level and lower: 1: error, 2: warning, 
                        3: info (default), 4: verbose, 5: debug. Default is 3, info. (This flag is equivalent to 
                        AGENT_VERBOSITY)
    -f, --overwrite     Install older version of the horizon agent and CLI packages. (This flag is equivalent to 
                        AGENT_OVERWRITE)
    -b, --skip-prompt   Skip any prompts for user input (This flag is equivalent to AGENT_SKIP_PROMPT)
    -C, --cli-only      Install only the horizon-cli package, not the full agent (This flag is equivalent to 
                        AGENT_ONLY_CLI)
        --container     Install the agent in a container. This is the default behavior for MacOS installations. 
                        (This flag is equivalent to ANAX_IN_CONTAINER)
    -A, --auto-upgrade  Auto agent upgrade. It is used internally by the agent auto upgrade process. (This flag is 
                        equivalent to AGENT_AUTO_UPGRADE)
    -h, --help          This usage
...
while getopts "c:i:j:p:k:u:d:z:hl:n:sfbw:o:O:T:t:D:a:U:CA-:" opt; do
    ...
    -)
        case "${OPTARG}" in
            ...
            tarfile)  
                val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                ARG_AGENT_INSTALL_ZIP="$val"
                ;;
            tarfile=*)  
                val=${OPTARG#*=}
                ARG_AGENT_INSTALL_ZIP="$OPTARG"
                ;;
            pubkey)  
                val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                ARG_PKG_APT_KEY="$val"
            ...
            user)  
                val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                ARG_HZN_EXCHANGE_USER_AUTH="$val"
                ;;
            user=*)  
                val=${OPTARG#*=}
                ARG_HZN_EXCHANGE_USER_AUTH="$val"
            ...
            auth)  
                val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                ARG_HZN_EXCHANGE_NODE_AUTH="$val"
                ;;
            auth=*)  
                val=${OPTARG#*=}
                ARG_HZN_EXCHANGE_NODE_AUTH="$val"
            ...
            wait)  
                val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                ARG_AGENT_WAIT_FOR_SERVICE="$val"
                ;;
            wait=*)  
                val=${OPTARG#*=}
                ARG_AGENT_WAIT_FOR_SERVICE="$val"
            ...
            service-org)  
                val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                ARG_AGENT_WAIT_FOR_SERVICE_ORG="$val"
                ;;
            service-org=*)  
                val=${OPTARG#*=}
                ARG_AGENT_WAIT_FOR_SERVICE_ORG="$val"
            ...
            timeout)  
                val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                ARG_AGENT_REGISTRATION_TIMEOUT="$val"
                ;;
            timeout=*)  
                val=${OPTARG#*=}
                ARG_AGENT_REGISTRATION_TIMEOUT="$val"
                ;;
            skip)  
                ARG_AGENT_SKIP_REGISTRATION=true
                ;;
            node-type)  
                val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                ARG_AGENT_DEPLOY_TYPE="$val"
                ;;
            node-type=*)  
                val=${OPTARG#*=}
                ARG_AGENT_DEPLOY_TYPE="$val"
            ...
            internal-url)  
                val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                ARG_INTERNAL_URL_FOR_EDGE_CLUSTER_REGISTRY="$val"
                ;;
            internal-url=*)  
                val=${OPTARG#*=}
                ARG_INTERNAL_URL_FOR_EDGE_CLUSTER_REGISTRY="$val"
            ...
            log=*)  
                ...
                ;;
            overwrite)  
                ARG_AGENT_OVERWRITE=true
                ;;
            skip-prompt)  
                ARG_AGENT_SKIP_PROMPT=true
                ;;
            cli-only)  
                ARG_AGENT_ONLY_CLI=true
                ;;
            container)
