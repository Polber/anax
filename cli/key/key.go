...
func Create(x509Org, x509CN, outputDir string, keyLength, daysValid int, importKey bool, privKeyFile string, pubKeyFile string, overwrite bool) error {
	...
	msgPrinter.Printf("Created keys:\n \t%v\n\t%v\n", privKeyFile, pubKeyFile)
...
func confirmOverwrite(privKeyFile string, pubKeyFile string, overwrite bool) {
	...
	if priveExists && pubExists {
		if !overwrite {
			cliutils.ConfirmRemove(msgPrinter.Sprintf("File %v and %v exist, do you want to overwrite?", privKeyFile, pubKeyFile))
		}
	} else {
		if priveExists && !overwrite {
			cliutils.ConfirmRemove(msgPrinter.Sprintf("File %v exists, do you want to overwrite?", privKeyFile))
		}
		if pubExists && !overwrite {
			cliutils.ConfirmRemove(msgPrinter.Sprintf("File %v exists, do you want to overwrite?", pubKeyFile))
		}
	}
